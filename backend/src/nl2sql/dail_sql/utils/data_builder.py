...
from src.nl2sql.dail_sql.utils.utils import get_tables_from_db, sql2skeleton
...
class BasicDataset:
    def __init__(self, path_data, pre_test_result=None):
        ...
        self.path_test_schema_linking = os.path.join(
            self.path_data, "enc/test_schema-linking.jsonl"
        )
        self.path_train_schema_linking = os.path.join(
            self.path_data, "enc/train_schema-linking.jsonl"
        )
    ...
    def get_pre_skeleton(self, queries=None, schemas=None, mini_set=False):
        if queries:
            skeletons = []
            for query, schema in zip(queries, schemas, strict=False):
    ...
    def get_train_json(self):
        datas = json.load(open(self.train_json))
    ...
    def get_test_json(self, mini_set=False):
        tests = json.load(open(self.test_json))
        if mini_set and self.mini_test_index_json:
            mini_test_index = self.get_mini_index()
        ...
        if self.pre_test_result:
            with open(self.pre_test_result) as f:
                lines = f.readlines()
    ...
    def get_train_schema_linking(self):
        if not os.path.exists(self.path_train_schema_linking):
            return None
        linking_infos = []
        with open(self.path_train_schema_linking) as f:
            for line in f.readlines():
                if line.strip():
                    linking_infos.append(json.loads(line))
        return linking_infos
    ...
    def get_train_answers(self):
        with open(self.train_gold) as file:
            answers = file.readlines()
            return answers

    def get_test_answers(self, mini_set=False):
        with open(self.test_gold) as file:
            answers = file.readlines()
            if mini_set and self.mini_test_index_json:
                mini_test_index = self.get_mini_index()
    ...
    def data_pre_process(self, datas, linking_infos=None, pre_queries=None):
        ...
        if linking_infos:
            ...
            for id in range(min(len(datas), len(linking_infos))):
                ...
                datas[id]["table_names_original"] = db_id_to_table_json[db_id][
...
class BullDataset(BasicDataset):
    name = "bull"
    test_json = "BULL-en/dev-preprocessed.json"
    test_gold = "dev_gold.sql"
    train_json = "BULL-en/train-preprocessed.json"
    train_gold = "train_gold.sql"
    table_json = "BULL-en/tables.json"
    mini_test_index_json = None
...
def load_data(data_type, path_data, pre_test_result=None):
    ...
    elif data_type.lower() == "bull":
        return BullDataset(path_data, pre_test_result)
    else:
        raise RuntimeError()
