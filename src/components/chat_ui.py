...
class QueryPilotChat:
    ...
    def process_user_question(self, question: str):
        ...
        sql_execution_result = self._execute_sql(final_sql_select, task_tracker, new_conversation_container)
    ...
    def _generate_sql(self, question: str, task_tracker: TaskTracker, container) -> Optional[str]:
    ...
    def _execute_sql(self, final_sql_select: str, task_tracker: TaskTracker, container) -> Optional[pd.DataFrame]:
                ...
                with st.spinner("⚡ Stage 2: Executing SQL query...", show_time=True):
                    try:
                        ...
                        if total_rows > self.MAX_ROWS:
                            st.warning(f"⚠️ Results are limited to {self.MAX_ROWS} rows out of {total_rows} total rows. Please refine your query for more specific results.")
    ...
    def _generate_response(self, sql_execution_result: pd.DataFrame, task_tracker: TaskTracker, container):
